...
change_sum_synt_shuff <- change_by_category_shuff %>%
  group_by(lexical_class, measure) %>%
  dplyr::summarise(mean = mean(value),
                   sd = sd(value),
                   n = n()) %>%
...
change_sum_synt_all <- change_sum_synt %>%
  mutate(corpus = 'real') %>%
  bind_rows(change_sum_synt_shuff %>%
              mutate(corpus ='shuffled'))
```
...
change_total <- change_by_category %>%
  group_by(measure, corpus) %>%
  dplyr::summarise(total = n())

change_sum_sem <- change_by_category %>%
...
change_total_shuff <- change_by_category_shuff %>%
  group_by(measure) %>%
  dplyr::summarise(total = n())

change_sum_sem_shuff <- change_by_category_shuff %>%
  group_by(lexical_class, category, measure) %>%
  dplyr::summarise(mean = mean(value),
                   sd = sd(value),
                   n = n()) %>%
  mutate(se = sd / sqrt(n),
